                  ...
                  <div className="space-y-0">
                      <table className="w-full border-collapse border border-black table-fixed text-[10px]">
                          ...
                          <tbody>
                            {pageRows.map((row) => {
                                if (row.type === 'header') {
                                    return <tr key={`header-${row.job.id}`} className="bg-slate-200 print:bg-slate-200"><td colSpan={13} className="border border-black px-2 py-1 align-middle"><div className="flex justify-between items-center font-bold text-xs uppercase tracking-wide"><span>{row.job.vesselName} ({row.job.voyageNo.slice(-3)})</span><div className="flex gap-4 font-mono"><span>VOY: {row.job.voyageNo}</span><span>ETA: {row.job.eta}</span></div></div></td></tr>;
                                } else {
                                    const item = row.data;
                                    return (
                                        <tr key={item.blId} className="align-top group/row">
                                            <td className="border border-black p-1 text-center align-middle font-bold relative group">{row.seqNo}<div className="absolute left-0 top-0 bottom-0 w-3 flex flex-col justify-center opacity-0 group-hover:opacity-100 transition-opacity print:hidden gap-0.5"><button onClick={(e) => { e.stopPropagation(); handleMoveRow(item.jobId, item.blId, 'up', summaryItems); }} className="h-3 flex items-center justify-center bg-slate-100 hover:bg-blue-100 text-slate-500 hover:text-blue-600 rounded-r"><ChevronUp size={8} /></button><button onClick={(e) => { e.stopPropagation(); handleMoveRow(item.jobId, item.blId, 'down', summaryItems); }} className="h-3 flex items-center justify-center bg-slate-100 hover:bg-blue-100 text-slate-500 hover:text-blue-600 rounded-r"><ChevronDown size={8} /></button></div></td>
                                            <td className="border border-black p-1 text-center align-middle whitespace-pre-wrap break-words font-medium leading-tight text-[9px]">{item.eta}</td>
                                            <td className="border border-black p-0 relative align-middle"><div className="flex flex-col h-full"><AutoResizeTextarea value={item.shipper} onChange={(e) => handleCellEdit(item.blId, 'shipper', e.target.value)} readOnly={isReadOnly} className="w-full bg-transparent border-b border-dashed border-gray-300 p-1 font-bold text-[10px] focus:bg-yellow-50 outline-none min-h-[30px]" placeholder="Shipper" /><AutoResizeTextarea value={item.description} onChange={(e) => handleCellEdit(item.blId, 'reportDescription', e.target.value)} readOnly={isReadOnly} className="w-full bg-transparent p-1 font-medium text-[9px] focus:bg-yellow-50 outline-none flex-1 min-h-[40px]" placeholder="Description" /></div></td>
                                            <td className="border border-black p-0 align-middle"><div className="flex flex-col h-full"><div className="flex-1 p-1 border-b border-dashed border-gray-300 flex justify-center items-center"><input className="w-full bg-transparent text-center font-bold outline-none focus:bg-yellow-50 text-[10px]" value={item.quantity} onChange={(e) => handleCellEdit(item.blId, 'quantity', Number(e.target.value))} readOnly={isReadOnly} /></div><div className="flex-1 p-1 flex justify-center items-center"><input className="w-full bg-transparent text-center outline-none focus:bg-yellow-50 text-[9px]" value={item.grossWeight} onChange={(e) => handleCellEdit(item.blId, 'grossWeight', Number(e.target.value))} readOnly={isReadOnly} /></div></div></td>
                                            ...
                                            <td className="border border-black p-1 text-center align-middle whitespace-pre-wrap break-all text-[8px] leading-tight font-mono"><span className="font-bold block mb-1">{item.blNumber}</span><span className="text-slate-600 block">{item.containerStr}</span></td>
                                            <td className="border border-black p-1 text-center align-middle"><div className="flex flex-col gap-1 text-[8px] font-bold"><span className={item.hasBL ? "text-black" : "text-gray-300"}>BL</span><span className={item.hasINV ? "text-black" : "text-gray-300"}>INV</span><span className={item.hasPL ? "text-black" : "text-gray-300"}>PL</span></div></td>
                                            <td className="border border-black p-0 align-middle"><AutoResizeTextarea value={item.itemVesselName} onChange={(e) => handleCellEdit(item.blId, 'note', e.target.value)} readOnly={isReadOnly} className="w-full h-full bg-transparent p-1 text-center focus:bg-yellow-50 outline-none text-[9px] break-words whitespace-normal" /></td>
                                            <td className="border border-black p-0 align-middle font-bold text-[9px]"><input className="w-full h-full bg-transparent text-center outline-none border-b border-dashed border-transparent focus:border-blue-500 focus:bg-yellow-50" value={item.typeAlias} readOnly={true} /></td>
                                            <td className="border border-black p-0 align-middle text-[9px]"><div className="flex flex-col p-1 gap-1"><div className="flex items-center gap-1"><span className="text-[8px] font-bold text-gray-500">FWD:</span><input className="flex-1 bg-transparent border-b border-gray-200 outline-none focus:bg-yellow-50 min-w-0" value={item.koreanForwarder} onChange={(e) => handleCellEdit(item.blId, 'koreanForwarder', e.target.value)} readOnly={isReadOnly} /></div><div className="flex items-center gap-1"><span className="text-[8px] font-bold text-gray-500">TRK:</span><input className="flex-1 bg-transparent outline-none focus:bg-yellow-50 min-w-0" value={item.transporter} onChange={(e) => handleCellEdit(item.blId, 'reportTransporter', e.target.value)} readOnly={isReadOnly} /></div></div></td>
                                            <td className="border border-black p-0 align-middle"><AutoResizeTextarea value={item.location} onChange={(e) => handleCellEdit(item.blId, 'reportStorageLocation', e.target.value)} readOnly={isReadOnly} className="w-full h-full bg-transparent p-1 text-center focus:bg-yellow-50 outline-none text-[9px] break-words" /></td>
                                            <td className="border border-black p-0 align-middle"><AutoResizeTextarea value={item.reportRemark} onChange={(e) => handleCellEdit(item.blId, 'reportRemarks', e.target.value)} readOnly={isReadOnly} className="w-full h-full bg-transparent p-1 text-center focus:bg-yellow-50 outline-none text-[9px] break-words" /></td>
                                        </tr>
                                    );
                                }
                            })}
